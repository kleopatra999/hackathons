...
app.controller('AuthController', function($q,$scope,$rootScope, $location, toaster,User, Auth) {
  ...
  $scope.login = function(user) {
       ...
       Auth.login.get({email:user.email, pw:user.password},
       function (resp) {
        console.log('Auth service returned:',resp)
                    ...
                    if (resp.rows) {
                              ...
                              if (User.isSignedIn) {console.log('Signed in!')};
...
$scope.register = function(user) { 
     ...
     var newUser = {username:user.name, email:user.email, pw:user.password} 
 ...
 $scope.changePassword = function(user) {
  var payload = {oldpw:user.oldPass, newpw:user.newPass, email:user.email}
  User.changePassword(payload).success(function(status){
    ...
    if (status=='OK') {
      ...
      else if (status == 401){toaster.pop('failed','Password change failed! Retry')}
  })
